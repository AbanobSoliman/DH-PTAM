...
class TUMVIEDataset(object):   # Stereo Event 3-Channel Tensors
    '''
    path example: 'path/to/your/TUM-VIE/mocap-desk2'
    '''
    def __init__(self, path, rectify=False):
        self.left_cam = Camera(
        ...
        path = os.path.expanduser(path)
        self.left = ImageReader(
            *self.list_imgs(os.path.join(path, 'result0', 'Left_E3CT')), 
            self.left_cam)
        self.right = ImageReader(
            *self.list_imgs(os.path.join(path, 'result0', 'Right_E3CT')), 
            self.right_cam)
        assert len(self.left) == len(self.right)
        self.timestamps = self.left.timestamps
    ...
    def list_imgs(self, dir):
        xs = [_ for _ in os.listdir(dir) if _.endswith('.png')]
        xs = sorted(xs, key=lambda x:float(x[:-4]))
        timestamps = [float(_[:-4]) * 1e-6 for _ in xs]
        return [os.path.join(dir, _) for _ in xs], timestamps
