...
sys.path.insert(0, './features/keynet/KeyNet/model/')
sys.path.insert(0, './features/keynet/KeyNet/aux/')
sys.path.insert(0, './features/keynet/KeyNet/datasets/')
sys.path.insert(0, './features/keynet/HSequences_bench/tools/')
# sys.path.append(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
from os import path, mkdir
import argparse
from skimage.transform import pyramid_gaussian
import geometry_tools as geo_tools
import repeatability_tools as rep_tools
import desc_aux_function as loss_desc
import hardnet_pytorch
import torch
...
def build_keynet_config(keynet_base_path):
    ...
    parser.add_argument('--pytorch-hardnet-dir', type=str,
...
def convert_pts_to_keypoints(pts, scores, sizes, levels):
    ...
    if pts is not None:
        ...
        kps = [cv2.KeyPoint(p[0], p[1], _size=sizes[i], _response=scores[i], _octave=levels[i]) for i, p in
               enumerate(pts)]
    return kps
...
class KeyNetDescFeature2D:
    def __init__(self,
        ...
        self.lock = RLock()
        self.model_base_path = "./features/keynet/"
    ...
    def load_model(self):
        ...
        with tf.name_scope('model_deep_detector'):
            ...
            output_network = deep_architecture.model(self.input_network, self.phase_train, self.dimension_image,
                                                     reuse=False)
            self.maps = tf.nn.relu(output_network['output'])

        # Extract Patches from inputs:
        self.input_patches = loss_desc.build_patch_extraction(self.kpts_coord, self.kpts_batch, self.input_network,
        ...
        for idx_level in range(self.levels):
            ...
            point_level.append(self.keynet_config.num_points * factor_points ** (
    ...
    def extract_keynet_features(self, image):
        pyramid = pyramid_gaussian(image, max_layer=self.keynet_config.pyramid_levels,
    ...
    def detectAndCompute(self, frame, mask=None):  # mask is a fake input
        with self.lock:
            ...
            if kVerbose:
                print('detector: KEYNET, descriptor: KEYNET, #features: ', len(self.kps), ', frame res: ',
                      frame.shape[0:2])
