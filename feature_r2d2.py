...
sys.path.insert(0, './features/r2d2/tools')
sys.path.insert(0, './features/r2d2/nets/')
import common
...
class R2d2Feature2D: 
    def __init__(self,
        ...
        self.lock = RLock()             
        self.model_base_path = "./features/r2d2"
        self.model_weights_path = self.model_base_path + '/models/faster2d2_WASF_N16.pt'
    ...
    def compute_kps_des(self,img):     
        with self.lock:        
            H, W = img.shape[:2]
            img = norm_RGB(cv2.cvtColor(img, cv2.COLOR_GRAY2RGB))[None]
